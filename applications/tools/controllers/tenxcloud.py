                ...
                if(len(services) > 0):
                    for service in services:
                        ...
                        for portMapping in service[api.KEY_PORT_MAPPING]:
                            db.port_mapping.insert(protocol=portMapping[api.KEY_PROTOCOL],url=portMapping[api.KEY_SERVICE_URL],port=portMapping[api.KEY_CONTAINER_PORT],service=serviceId)
    return "ok"
